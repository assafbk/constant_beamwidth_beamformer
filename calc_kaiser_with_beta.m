%{
 calculates the kaiser window for the given beta.
%}

function w_kaiser = calc_kaiser_with_beta(beta,M)
...
%     w_kaiser(:,i) = w_kaiser(:,i)/sum(w_kaiser(:,i));  % normalize
    ...
    w_kaiser = besseli(0,beta*sqrt(1-(m/N).^2))/besseli(0,beta);
    w_kaiser = w_kaiser/sum(w_kaiser);  % normalize
    w_kaiser = w_kaiser.';
